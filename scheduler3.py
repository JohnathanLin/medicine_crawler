...
def get_document(page_num, thread_number, collection_name):
    ...
    """
    # 获取省份和城市信息
    parent_url = 'https://www.yaofangwang.com/catalog-1-p'
    # 将所有Url装进队列，用于多线程爬取
    urlQueue = queue.Queue()
    for page in range(page_num):
        # url_queue.put(url_obj['url'])
        url = parent_url + str(page + 1)+'.html'
        urlQueue.put(url)
    threads = []
...
def geturl():
    ...
    url_mongo_collection = "3_url"
    mongo_url_detail_collection = '3_url_detail'
...
def getdetail():
    ...
    collection_name = "3_medicine_row_json"
    # print(len(url_list))
    # 将所有Url装进队列，用于多线程爬取
    urlQueue = queue.Queue()
